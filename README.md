...
* Download and use FastX3 desktop client to connect to Quest
...
```$ module load deeplabcut/2.2.0.2```
 ```$ python3 -m deeplabcut```
    * These commands must be ran everytime a new quest session is started
 
## Creating a New Project:
* Navigate to the "Manage Project" tab
* Type in the project name, experimenter name, and load all the videos that will be analyzed.
...
* Check to see if a /labeled-data/ can be found within your project directory. 
...
* Load the frames you would like to label
    * Right click to label the body part that is selected (Look at panel on the right)
    * To an already placed label, left click and drag to desired spot
    * Center click to remove label
* When labeling:
    * Be as accurate as possible with your labeling
    * If bodypart(s) cannot be seen clearly in a frame, move on next frame. It is better to leave a frame unlaballed compared to labeling it incorrectly
    * Make sure to save before moving on the next frame
* Repeat this process till atleast 200 frames (10 videos) have been labeled
    * The more frames labeled, the more accurate the model will be
* After labeling the 20 frames from a video, move onto next set of 20 frames from next video
    * At the start every new video, make sure to see that the a CollectedData_[name].csv is file is created within the /labeled-data/[video_name]/ directory **AND** if the data is being logged properlly into the csv everytime labels are saved
    * If .csv file is not create OR the data is not being logged properly, close out the labeling window and reopen
...
* Open the terminal again, enter the following commands:
    ```$ module load deeplabcut/2.2.0.2```
    ```$ python3```
...
* Run ```deeplabcut.train_network(path_config_file, max_snapshots_to_keep=100, maxiters=1000000)```
    ...
    * Stop the training whenever desired, the more iteration the better.
...
* This will produce a labeled video and csv file with all the estimated datapoints of each bodypart
* If satified, use a bash script to produce all other labeled videos and .csv files:
```bash=
#!/bin/bash
